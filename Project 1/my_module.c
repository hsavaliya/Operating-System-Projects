...
asmlinkage size_t our_sys_read(int fd,char __user *buf,size_t count)
{
	...
	char fileinfo_buff[300], fdbuffer[100], cntbuffer[100],path[120],procPath[120], readfilepath[100],  fdkey[12];
	int ret;
	char filepath[100];
	ssize_t len=0;
...
void write_proc_file(char *buffer,char *path)
{
	...
	set_fs(old_fs);
		...
		//write_info(proc_entry,buffer,strlen(buffer), NULL);
...
ssize_t write_info( struct file *filp, const char __user *buff, unsigned long len, void *data )
{
    ...
    if (len > capacity)
    {
        printk(KERN_INFO "No space to write in myModule!\n");
        return -1;
    }
...
static int init(void)
{
	...
	/* create a Proc file */	
	    ...
	    proc_entry = create_proc_entry( "myModule", 0677, NULL );

	    if (proc_entry == NULL)
	    {
		ret = -1;
		vfree(info);
		printk(KERN_INFO "myModule could not be created\n");
	    }
	    else
	    {
		write_index = 0;
		...
		printk(KERN_INFO "myModule created.\n");
	    }
...
static void exit(void)
{
	...
	kfree(hashtable);
	remove_proc_entry("myModule", proc_entry);
